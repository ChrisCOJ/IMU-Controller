...
{
    // Setup
    master_init(MPU6050_I2C_ADDR);  // Init i2c bus, master device, and specified slave device
    ...
    // Read the value of the mpu6050's accel config register to ensure correct initialization
    uint8_t accel_config_buf[1];
    esp_err_t err = mpu_read_reg(dev_handle, MPU6050_ACCEL_CONFIG_REG, accel_config_buf, sizeof(accel_config_buf));

    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Value of accel config register: %u", accel_config_buf[0]);
    }
    ...
    while(10) {
        ...
        for (int i=0; i < raw_accel_arr_size; ++i) {
            ...
            ESP_LOGI(TAG, "%s: %d", axes[i], raw_accel_arr[i]);
        }
        ESP_LOGI(TAG, "");
